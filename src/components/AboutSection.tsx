...
const AboutSection = () => {
  const techStack = [
    ...
    {   
      name: 'GitHub', 
      icon: <Github size={24} className="text-gray-700" />, 
      category: 'devops' 
    },
    { 
      name: 'Docker', 
  ...
  return (
    <section id="about" className="min-h-screen py-8 bg-[#0B1121] text-white">
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="text-center mb-6">
          <p className="text-[#0EA5E9] text-sm font-medium mb-1 mt-7">ABOUT ME</p>
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-2">Know Me More</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-6">
            ...
            <div className="bg-[#0F172A] p-4 rounded-lg">
              ...
              <div className="bg-[#1E293B] p-4 rounded-lg border-l-4 border-[#0EA5E9]">
              ...
              <div className="grid grid-cols-4 gap-2">
                {techStack.map((tech) => (
                  <div 
                    key={tech.name}
                    className={cn(
                      "bg-[#1E293B] p-2 rounded-lg",
                      "flex flex-col items-center justify-center",
                      "hover:bg-[#2D3B4F] transition-colors duration-300",
                    )}
                  >
                    <div className="w-6 h-6 flex items-center justify-center mb-1">
                      {React.cloneElement(tech.icon, { size: 16 })}
                    </div>
                    <span className="text-[10px] font-medium text-center text-gray-300">{tech.name}</span>
                  </div>
                ))}
              </div>
            </div>
